...
const { isLoggedIn , catogaries, OnAddPost } = useContext(AuthContext)
const [chosenCats, setChosenCats]=useState([])
const [errorAddPost , setErrorAddpost] = useState({message: "", isError : false})
...
const formIsValid = titleIsValid && enterePostIsValid &&  chosenCats.length > 0 ? true : false;
...
const titleInputClasses =  titleInputHassError 
? `${classes.invalid} `
: "";

const pastInputClasses =  postInputHassError
? `${classes.invalid} `
: "";
...
<Form onSubmit={handleSubmit} className="py-3"  > 
   ...
   { titleInputClasses  &&  <p className={classes['error-text']} >input title must not be emty.</p>}
   ...
   { pastInputClasses  &&  <p className={classes['error-text']} >input text must not be emty.</p>}
  ...
  <div className="mt-4">
      ...
      {catogaries.length> 0 &&  catogaries?.map(el =>  
...
{ errorAddPost.isError  &&  <p className={classes['error-text']} >{errorAddPost.message}.</p>}
  </MyModal>
)
