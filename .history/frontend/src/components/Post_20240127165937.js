...
import {AiOutlineLike, AiOutlineDislike , AiOutlineComment, AiFillDelete} from "react-icons/ai";
...
const Post =  ({id, title, body, categories, created_at, comments, likes, dislikes, username, likeHandler,disLikeHandler})=>{
   const [ShowComment, setShowComment] = useState(false)
   const {isLoggedIn, username: LoggedInUser, onRemovePost} = useContext(AuthContext)
...
<span>categories</span> {categories.map(el=> (<span key={el} className={classes.cat}>{el}</span>))}
</div>

<div className={`${classes["post-reaction-info"]} d-flex justify-content-between`}>
{likes > 0 &&<span>{likes} likes</span>}
 {dislikes > 0 &&<span> {dislikes} dislikes</span>}
{comments?.length > 0 && <span onClick={()=> setShowComment(true)}> {comments?.length} comments</span>}
</div>

<div className={`${classes["post-reaction"]} d-flex justify-content-between`}>
