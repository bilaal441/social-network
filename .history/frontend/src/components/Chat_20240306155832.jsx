...
const ChatComponent = () => {
  ...
  const fetchMoreMessages = async () => {
    if (!fetchInProgress && messages.length < 100) {
      setFetchInProgress(true); // Set fetchInProgress to true while fetching
      ...
      setTimeout(() => {
        ...
        setFetchInProgress(false); // Reset fetchInProgress after fetching
      }, 500);
    } else if (!fetchInProgress && messages.length >= 100) {
      setHasMore(false);
    }
  };

  const throttleChatScrollHandler = throttle(async (e) => {
    const el = e.target;
      ...
      const fetchMoreMessages = async () => {
        if (!fetchInProgress && messages.length < 100) {
          ...
          setTimeout(() => {
            ...
            // Scroll to the first of the last messages
            if (endMessageRef.current) {
              endMessageRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
            }
          }, 500);
        } else if (!fetchInProgress && messages.length >= 100) {
          setHasMore(false);
        ...
        >
          ...
          {messages.map((message, i) => (
            <React.Fragment key={message.id}>
              <div
                ref={i >= messages.length - 10 ? fistOfTheLastTen : null}
                className="chats"
              >
