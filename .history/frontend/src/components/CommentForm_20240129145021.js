...
const Comment = ({comments, id: postId}) => {
  ...
  return (
    <div className="comment mt-3">
      ...
      <div className={cl["comment-container"]}>
        {comments?.map(
          ({id, created_at, username, body, likes, dislikes}, i) => (
            <div className={cl.comment} key={id}>
              <div className={cl["comment-header"]}>
                <span className="user name">
                  posted by {username} {moment(created_at).fromNow()}
                </span>
              </div>

              <div className={cl["comment-body"]}>{body}</div>

              <div className={cl["react-status"]}>
                <span>{likes} likes</span>
                <span>{dislikes} dislikes</span>
              </div>

              <div className={cl["comment-reaction"]}>
                <button
