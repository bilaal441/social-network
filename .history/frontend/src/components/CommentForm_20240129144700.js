...
import classes1 from "./CommentForm.module.css"
...
const [addCommentErr, setAddCommentErr] = useState({message: "", isError: true})
const {OnAddCommentToPost, isLoggedIn, onAddLikeDislikeComment} =useContext(AuthContext)
...
<Form.Group controlId="formBasicText" className={commnetInputClasses}>
   ...
   {addCommentErr.isError  &&  <p className={classes['error-text']} >{addCommentErr.message}</p>}
...
<div className={classes1['comment-container']}>
  {
    comments?.map(({ id, created_at, username, body, likes, dislikes}, i)=> 
    <div className={classes1.comment} key={id}>
      <div className={classes1['comment-header']}>
      <span className="user name">
     posted by {username}  { moment(created_at).fromNow()}
  </span>
    </div>

     <div className={classes1['comment-body']}>
       {body}
       </div>
         
       <div className={classes1['react-status']}>
         <span>
           {likes} likes
         </span>
         <span>
       ...
       <div className={classes1['comment-reaction']}>
          <button onClick={likeHandler.bind(null, {commentId: id, postId})} >
          <AiOutlineLike />  like
          </button>
          <button onClick={disLikeHandler.bind(null, {commentId: id, postId})} >
