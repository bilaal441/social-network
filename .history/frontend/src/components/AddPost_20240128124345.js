...
const AddPost = () => {
  ...
  const [isRighprivacy, setIsRighprivacy] = useState("");
  ...
  useEffect(() => {
    ...
    if (privacy === "almost" && suggestions.length < 0) {
  ...
  const formIsValid = titleIsValid && enterePostIsValid  && isRighprivacy? true : false;
      ...
      <Form onSubmit={handleSubmit} className="py-3">
        ...
        <Form.Group controlId="privacy">
          ...
          <Form.Select value={privacy} onChange={handlePrivacyChange}>
            ...
            <option value="Allmost">Almost private</option>
          </Form.Select>
        </Form.Group>
        ...
        <ul>
          {suggestions.map((suggestion, index) => (
            <li key={index}>{suggestion.username}</li>
          ))}
        </ul>
