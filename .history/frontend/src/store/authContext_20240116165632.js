...
const { comments,   ...rest} = posts.find(el => el.id ===postId)
   if(comments === null){
   SetPosts( posts.map(el=> el.id === postId ? {
    comments:[Comment],
    ...rest
   }   : el  ))
   }else{
...
const onAddLikeDislikeComment = ({commentId, postId}, data, quary)=>{
  ...
  if(quary === "like" || "dislike"){
    const { comments,   ...rest} = posts.find(el => el.id ===postId)
     ...
     return el.id === commentId ? { likes: data.likes, dislikes: data.dislikes  ,...all} : el
    } )
       SetPosts( posts.map(el=> el.id === postId ? { comments: upDatedComments, ...rest}   : el  ))
    }
