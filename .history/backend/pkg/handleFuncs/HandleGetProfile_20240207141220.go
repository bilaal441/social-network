...
func GetProfileOwner(userId string) (User, error) {
	profile := User{}
	err := database.QueryRow("SELECT * FROM Users WHERE id = ?", userId).Scan(&profile.Id, &profile.Nickname, &profile.FirstName, &profile.LastName, &profile.Email, &profile.Password, &profile.Avatar, &profile.About, &profile.PrivacySetting, &profile.DOB, &profile.CreatedAt)
