...
func HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// var storedPassword string
		err := database.QueryRow("SELECT Id, Password, Nickname,  Profile  FROM Users WHERE Nickname=? OR Email=?", entredData.Nickname, entredData.Email).Scan(&id, &storedPassword, &username, &imgUrl)
		if err != nil {
