...
func GetProfileOwner(userId string) (User, error) {
	profile := User{}
	err := database.QueryRow("SELECT * FROM users WHERE id = ?", userId).Scan(&profile.Id, &profile.Nickname, &profile.FirstName, &profile.LastName, &profile.Email,
		&profile.Password, &profile.Avatar, &profile.Aboutme, &profile.PrivacySetting, &profile.DOB, &profile.Gender, &profile.CreatedAt
	)
	if err != nil {
		return profile, err
	}
