...
func GetProfileOwner(userId string) (User, error) {
	profile := User{}
	err := dat.QueryRow("SELECT * FROM users WHERE id = ?", userId).Scan(&profile)
	if err != nil {
		return profile, err
	}
	return profile, nil
...
func GetFollowersOrFollowing(ownerId string, itemId string, offset int) ([]string, error) {
	...
	rows, err := dat.Query(query, ownerId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetNicknameFromId(userId string) string {
	var nickname string
	err := dat.QueryRow("SELECT Nickname FROM users WHERE id = ?", userId).Scan(&nickname)
	if err != nil {
		return ""
	}
	return nickname
...
func GetPosts(userId string, page int, batchSize int, usersOwnProfile bool) ([]Post, error) {
    ...
    LIMIT ? OFFSET ?
`
	rows, err := dat.Query(query, userId, batchSize, offset)
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func CheckSuperprivateAccess(postId string, userId string) (bool, error) {
	var exists int
	err := dat.QueryRow("SELECT 1 FROM PostAccess WHERE PostId = ? AND UserId = ?", postId, userId).Scan(&exists)
	if err != nil {
...
func GetUserIdFromCookie(sessionId string) (string, error) {
	var userId string
	err := dat.QueryRow("SELECT UserId FROM Sessions WHERE Id=?", sessionId).Scan(&userId)
	if err != nil {
		return "", err
	}
	return userId, nil
