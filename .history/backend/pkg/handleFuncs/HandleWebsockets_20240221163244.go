...
func HandleConnection(w http.ResponseWriter, r *http.Request) {
	...
	// this.
	broadcastUserList()
	fmt.Println("HandleConnection 202")
camelsBack:
	for {
...
func closeConnection(conn *websocket.Conn) {
	...
	// Add a delay to give the client a chance to close the connection
	time.Sleep(1 * time.Second)
	if err := conn.Close(); err != nil {
		if !websocket.IsCloseError(err, websocket.CloseNormalClosure, websocket.CloseGoingAway) {
			fmt.Println("Error closing websocket hx.h.k h/lksch .lksh:", err)
		}
	}
}
...
func logout(userID string, thisConn *websocket.Conn, w http.ResponseWriter) {
	...
	for _, c := range activeConnections[userID] {
		if c != thisConn {
			...
			fmt.Println("thisConn::::::::", thisConn, "c:::::::", c)
