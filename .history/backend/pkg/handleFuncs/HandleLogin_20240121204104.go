...
func HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if err != nil {
			ferr := database.QueryRow("SELECT Id, Password, Nickname,  Profile,  FROM Users WHERE Email=?", entredData.Email).Scan(&id, &storedPassword, &username, &imgUrl)mt.Println(err.Error(), "error after getting data")
