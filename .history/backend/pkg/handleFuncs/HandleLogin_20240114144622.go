...
func HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// var storedPassword string
		err := database.QueryRow("SELECT Id, assword, Nickname,  profileImg  FROM Users WHERE nickname=? OR email=?", entredData.Nickname, entredData.Email).Scan(&id, &storedPassword, &username, &imgUrl)
		if err != nil {
