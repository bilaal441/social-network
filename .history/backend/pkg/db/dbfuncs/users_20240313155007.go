...
func GetUserById(id string) (User, error) {
	...
	err := db.QueryRow("SELECT Id, Nickname, FirstName, LastName, Email, Password, Avatar, AboutMe,, DOB, CreatedAt FROM Users WHERE Id=?", id).Scan(&user.Id, &user.Nickname, &user.FirstName, &user.LastName, &user.Email, &user.Password, &user.Profile, &user.AboutMe, &user.PrivacySetting, &user.DOB, &user.CreatedAt)
